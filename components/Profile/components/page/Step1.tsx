...
function InstructionBar({title,id,setPage1Data,page1Data,handleTitleError}: InstructionBarPropsType) {
  ...
  return (
    <div className="flex  items-center space-x-4 bg-white shadow px-4 py-3 rounded-lg border relative hover:border-orange" >
      <AiOutlineCheck className={`bg-green-500 text-white rounded-full text-lg ${title === page1Data.service_title  ? "visible" : "invisible" } absolute left-2`}/>
      <label htmlFor={id.toString()} className={`cursor-pointer text-sm   w-full`}>
        <input type="radio" value={title} id={id.toString()} name="service_title" className="peer sr-only" onChange={handleChange} checked={title === page1Data.service_title}/>
...
export default function Page1({setPage1Data,page1Data,titleError,handleTitleError,serviceTitle}:PagePropsType) {
    ...
    <div className="sm:pt-2 pt-1 px-4 rounded-b-md">
      <div className="py-1">
        <div>
          ...
          What specific kind of home renovation work are you looking for?
          </h2>
          <ul>
          {titleError !== ''&&
          <div className="bg-red-500 lg:w-[32.5%] p-2 rounded-md mt-2 mb-3 ">
            <span className="text-white  text-sm md:text-[15px] font-semibold">{titleError}</span>
          </div>
              }
            <ul className={` ${ ExampleDataForTitle.length <= 7 ? "md:grid-cols-2" : "lg:grid-cols-3" } grid-cols-1 md:grid-cols-2   grid gap-5 justify-center items-center relative mb-8 mt-1`}>
              {serviceTitle.map((item,index) => (
                <InstructionBar key={index} title={item} id={index+1}  setPage1Data={setPage1Data} page1Data={page1Data} handleTitleError={handleTitleError} />
              ))}
            </ul>
            {/* Constant */}
            <div className="space-y-5 mt-5 mb-5">
              <div className="flex gap-5 flex-wrap">
                ...
                <div className="flex justify-between items-center bg-white shadow px-4 py-3 rounded-lg border">
                  <input type="text" placeholder="Enter other title" name="other_title" className="outline-none" onChange={handleChangeInput} value={page1Data.other_title}/>
                </div>
              </div>
            </div>
          </ul>
