...
export default function Service({
  ...
  const [working_SchedulePage, setWorking_SchedulePage] = useState<
    "flexible" | "quickly" | "in_a_week" | "1n_a_3_month"
  >("flexible"); //page6 data
        ...
        try {
          ...
          if (status === 200) {
            if (data.role === "handyman") {
              handleError("Only Clients can Post Jobs");
            } else {
              console.log("Step 5 try 200");
              setIsNextBtnDisable(true);
              toast.success("User already exist.Please Enter your password");
  ...
  async function handleSendJOB() {
    if (currentStepIndex === 5) {
      ...
      if (contactDetailsPage.password.length < 8)
        return setContactDetailsPageError((e) => ({
          ...e,
          emailError: "Password must be at least 8 character",
        }));
    }
    // console.log('sdfasdf')
