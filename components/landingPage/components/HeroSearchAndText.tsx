...
export default function HeroSearchAndText({
  ...
  useEffect(() => {
    ...
    setUserType([searchDefaultField]);
  ...
  return (
    ...
    >
      {title && (
        <h1
          className={`${homePageOrNOt ? "sm:text-[4rem]" : "text-center"
            } text-5xl font-bold leading-tight `}
        >
          Encuentra manita profesional para todos tus{" "}
          <span
            className={`text-orange  text-5xl ${homePageOrNOt && "sm:text-[4rem]"
              } font-bold`}
          >
      ...
      <div className={`mb-6 ${homePageOrNOt ? "mr-20  w-full" : "w-3/4"}`}>
        ...
        <div className="bg-white relative mt-3">
          ...
          <div
            className={`border-2 bg-white py-5 shadow-md rounded-b-md  w-full absolute z-20 top-12 ${userType.length !== 0 && userType[0] !== "" ? "block" : "hidden"
              }`}
          >
            {Services.length !== 0 ? (
