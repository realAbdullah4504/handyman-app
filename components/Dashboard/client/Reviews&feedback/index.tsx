...
const ReviewsSection = ({ review: item }: { review: Review }) => {
	...
	const handleEdit = async ({
		...
		} else toast.error("Can not update more than once");
	};
		...
		<div
			...
			<section className="flex items-center justify-between p-1">
				...
				<div className="font-semibold flex flex-col text-sm">
					Dated : {moment(review.createdAt).format("Do MMM YY")} at{" "}
					{moment(review.createdAt).format("LTS")}{" "}
					{moment(review.createdAt).format("LTS") !==
						moment(review.updatedAt).format("LTS") && (
						<span>
							Edited:{" "}
							{moment(review.updatedAt).format("Do MMM YY")} at{" "}
							{moment(review.updatedAt).format("LTS")}{" "}
						</span>
					)}
				</div>
			...
			<div className="flex relative md:justify-around md:items-center gap-16 px-7  pb-5 flex-col md:flex-row">
				<div className="flex flex-col justify-center items-center">
					<span>{review.rating}.0 stars</span>
					<div className="flex">
						{Array.from({
							length: 5,
						})
			...
			<CustomConfirmPrompt
				...
				promptText="Are you sure want to deactive this review?"
			/>
		</div>
	);
};
	...
	return (
		<div className="my-2 w-full md:mx-32 min-h-[90vh]">
			<h1 className="text-3xl font-bold">
				<span className="text-orange">Real Opinions:</span> Discover
			...
			<div className="my-7 mx-5">
				{data?.pages[0]?.data.length > 0 ? (
					...
					<NotFoundData text="No reviews or feedback provided yet" />
				)}
