...
export default function AcceptOffers({ item: data }: { item: OfferItem }) {
	...
	const [isGivingRaview, setIsGivingReview] = useState(false);
		...
		try {
			await UpdateJobOffer.mutateAsync(
				...
				},
				{
					onSuccess(data) {
						setItem(data);
						toast.success(`Offer request ${status}`);
					...
					<section>
						...
						<div className="flex items-center">
							{item?.craftman?.reviews ? (
								...
								<span className="font-semibold">No review</span>
							)}
