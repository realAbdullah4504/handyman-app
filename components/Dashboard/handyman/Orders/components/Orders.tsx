		...
		<>
			<div className="w-full py-5 px-3">
				<section className="flex flex-col md:flex-row justify-between items-center">
					...
					<div className="w-full md:w-5/5 flex flex-col md:flex-row justify-between items-center mt-3 md:mt-0">
						<div className="text-normal md:w-2/3 md:text-left">
							{title}
						</div>
						<div className="font-bold md:w-1/3 md:text-right">{`Listing ID: ${listingId}`}</div>
						<div className="flex items-center w-full gap-4 mt-3 md:mt-0 md:w-1/3 md:justify-end">
							{status === "pending" && (
								<button
									onClick={() => setIsOfferWithdraw(true)}

									className="bg-orange px-2 mx-1 rounded  text-[14px] py-3 font-semibold ">
									Oferta de retiro
								</button>
							)}
							<StatusButton
