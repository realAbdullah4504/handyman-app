      ...
      {testData.map((item) => (
        ...
        <div className="flex justify-center items-center flex-col space-y-7">
          <section className="px-10 space-y-5">
            <h1 className="text-xl text-center font-bold">{item.month} <br/> Subscription</h1>
            <p className="text-3xl text-center font-semibold">{item.price}</p>
          </section>
          <button className="bg-white border-2 border-black rounded-lg px-7 py-1.5 hover:border-gray-500">Payment</button>
        </div>
        </div>
      ))}
    </div>
