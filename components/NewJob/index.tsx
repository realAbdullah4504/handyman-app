...
export default function Job() {
	...
	const [getSingleJob, setGetSingleJob] = useState<SingleJobData | null>(
		null
	);
	...
	const handleMessageSent = async () => {
		...
		} else toast.error("Please Login as Craftman before sending a message");
	};
	...
	const handleMakeOffer = async () => {
		try {
			if (user?.role === "handyman") {
				...
				toast.error("Please Login as Craftman before sending a Offer");
		} catch (error) {
			...
			<div className="w-full mt-10">
				{user?.craftsman?.status === "unverified" && (
					<p className="text-center bg-orange text-white rounded w-full p-4 text-2xl mb-5">
						Your profile is not yet activated
					</p>
				)}
				{/* Job details section */}
				{getSingleJob && (
								...
								<section className="py-3 px-4">
									...
									<section className="w-5/5 flex justify-between">
										...
										<section className="space-y-2 mt-2 w-3/5">
											...
											<p>
												{
													getSingleJob?.additional_job_description
												}
											</p>
										</section>
									</section>
									...
									<div className="flex gap-5 items-center justify-between py-2">
										<div className="flex justify-center items-center gap-2">
											...
											<span>
												{" "}
												{user &&
													`Distance: ${getSingleJob?.distance.toFixed(
														2
													)}
									km`}
											</span>
			...
			<ModalStruc
				...
				<div>
					<p className="mb-4 text-[20px] font-semibold">
						Paid subscription is required for this feature
					</p>
			...
			<ModalStruc
				...
				<div>
					<p className="mb-4 text-[20px] max-w-[300px] text-center bg-orange text-white rounded p-4 shadow">
						Your profile is not yet activated. You can use this
						function as soon as your profile has been activated
					</p>
				</div>
			</ModalStruc>
		</>
