	...
	try {
		...
		if (token?.role === "admin") {
			...
			(!payload?.user || !payload?.status) &&
				createError("required params 'status or user' is missing", 400);

			if (payload?.status === "declined" && !payload?.message) {
				createError("required params 'message' is missing", 400);
			}
				...
				content: {
					text:
						result.status === "verified"
							? "Congratulations! Your verification is completed"
							: `We are sorry! Your verification is declined. Reason: ${result.message}`,
					senderName: "",
