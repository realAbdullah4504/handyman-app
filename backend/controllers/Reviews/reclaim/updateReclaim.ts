...
const updateReclaim = async (req: NextApiRequest, res: NextApiResponse) => {
	try {
		...
		!reclaimId && createError("invalid request", 400);

		// Handling missing required params if decision is missing
		!decision && createError("missing required params", 400);
		...
		!type && createError("required params is missing", 400);
		...
		if (token?.role === "admin") {
			...
			const Noti = {
				...
				content: {
					text:
						type === "rejected"
							? "Your reclaim request is rejected.Check details"
							: "Your reclaim request is accepted",
					senderName: "",
