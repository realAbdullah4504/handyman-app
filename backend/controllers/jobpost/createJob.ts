...
const createJob = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const createJob = async (userData: any) => {
      ...
      if (!location) return createError("location is required", 400);
      const newJobData = {
        ...newData,
        listingId,
        location: {
      ...
      const getRes = async (err: any, decoded: any) => {
        ...
        if (userData !== "client") {
          return res.status(401).json({
            info: "Only Clients can Post Jobs",
            success: 0,
            status: 401,
          });
        }
    ...
    if (!userData) {
      ...
      return res.status(200).json({
        info: "We have sent an email. Please verify your account",
        success: 1,
        status: 200,
    ...
    if (!userData.status) {
      ...
      return res.status(200).json({
        info: "We have sent an email.Please verify your account first",
        success: 1,
        status: 200,
      });
    }
    ...
    if (passwd && userData) {
      ...
      return res.status(401).json({
        info: "Invalid password! Please login with valid password",
        success: 0,
        status: 401,
      });
    }
