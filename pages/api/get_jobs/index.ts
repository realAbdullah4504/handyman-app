	...
	try {
		if (req.method === "GET") {
			try {
				...
				let jobPosts = await JobPost.aggregate(
					aggreationPipeline
				).exec();
				
				// Send response with fetched job data and pagination metadata
				res.status(200).json({
					message: "Public job data retrieved",
					data: jobPosts,
					totalPages,
					currentPage,
				});
		...
		if (req.method === "POST") {
			...
			if (status) {
				...
				const jobDetails = await JobPost.findById(
					findCriteria
				).updateOne({
					$set: { status: status },
				});
			...
			if (newTitle || newParagraph) {
				...
				const updatedJob = await JobPost.findById(
					findCriteria
				).updateOne({
					$set: updateCriteria,
				});
